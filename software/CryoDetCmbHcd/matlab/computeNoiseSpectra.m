function computeNoiseSpectra(path2outputs,filelist,var2add,text2add,plotEtaScan)
    savePSD=true;

    if isstr(filelist)
        runtxt=fileread(filelist);
    ...
    %%
    fileID2=fopen(fullfile(path2outputs,'noise.txt'),'at');
    ...
    for fidx=1:Nfiles
        ...
        loglog(f,medfilt1(pArtHz,3)); hold on;
        ...
        text(5,100,text2add,'color','black','FontSize',16);
        ...
        text(250,0.075,text2add,'color','black','FontSize',16);
        ...
        if plotEtaScan
            ...
            fprintf(fileID2,'%d\t%d\t%d\t%0.3f\t%0.3f\t%0.3e\t%0.3e\t%0.2f\t%0.2f\t%0.3e\t%0.3e\t%0.2f\t%0.2f\t%0.3e\t%0.3e\t%0.3e\t%0.3e\t%0.3e\t%0.3e\t%0.3f\t%0.1f\t%s\n',cfg.readoutChannelSelect,band,subband,Foff,F,fsubset_min,fsubset_max,psd_mean_in_subset,psd_median_in_subset,fsubset_min2,fsubset_max2,psd_mean_in_subset2,psd_median_in_subset2,etaOut.R,min_resp,max_resp,p(1),p(2),latency, mean(amp)/1e3, dFr*1000.,var2add);
        else
            fprintf(fileID2,'%d\t%d\t%d\t%0.3f\t%0.3f\t%0.3e\t%0.3e\t%0.2f\t%0.2f\t%0.3e\t%0.3e\t%0.2f\t%0.2f\t%0.3e\t%0.2f\t%s\n',cfg.readoutChannelSelect,band,subband,Foff,F,fsubset_min,fsubset_max,psd_mean_in_subset,psd_median_in_subset,fsubset_min2,fsubset_max2,psd_mean_in_subset2,psd_median_in_subset2, mean(amp)/1e3, dFr*1000.,var2add);           
        end    
        hold off;
    end
    fclose(fileID2);
